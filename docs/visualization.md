...
- **球具规范**：1/9 黄、2/10 蓝、3/11 红、4/12 紫、5/13 橙、6/14 绿、
  7/15 枣红、8 黑；1–8 为实色，9–15 为白底同色腰带。号码圆标始终保留，不能只靠颜色区分。
- **球杆**：默认总长 1.4m，按 `intentAngle` 放在母球反向延长线上，击球前停留一小段时间；
  木质主体不变，A/B 选手色同时用于握把和靠近母球的杆头短节，确保杆尾出画时仍可辨认。
- **阻尼感**：播放器严格沿物理 sample 时间推进，不额外套 easing；通过球体接触阴影和由相邻
  物理帧速度推导的短运动尾迹表达重量，速度下降时尾迹自然收短。减少动态模式不显示尾迹。
- **稳定场景图**：球桌 SVG、袋口、球杆和各球节点整局只创建一次；动画帧仅更新球节点
  `transform` 与尾迹属性，进袋球切换显隐。禁止在 requestAnimationFrame 中使用 `innerHTML`
  重建球桌，球权切换不得产生空白帧。
- **观战控制条**：球桌上方提供开新局、暂停、重播、倍速、最大杆数和全屏。全屏时使用
  实心深色底部控制条，鼠标静止 1.8 秒后隐藏，键盘焦点或暂停时保持可见；触控目标至少 44px。
- **AI 本杆思路**：右栏先显示计划再播放轨迹，动画结束后才展示服务端复盘，避免提前剧透。
  计划由观察/判断/走位/风险/信心组成；最近事件占据剩余纵向空间并使用细分隔线，不套卡片。
- **固定观战屏**：桌面端 body 与主网格固定在视口内，AI 计划和最近事件各自滚动；事件按
  最新在上排列。窄屏保持固定外壳，由 `.main` 作为唯一主内容滚动区，避免多层页面滚动。
