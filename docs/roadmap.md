...
- [x] **M6.4 常驻大厅 v1**（2026-08-30）：单端口多桌常驻、动态认座（身份名自带）、
  handSeed 跨局肌肉记忆、终局自动续局、选桌观战、等候名单（只读）——docs/lobby.md
- [ ] 常驻服务 v2：大厅鉴权、自动补位调度、断线重连
- [x] Agent vs Agent 对弈（Elo 榜）（2026-08-30：大厅终局自动入账，SQLite 持久，零和）
- [ ] Hustle 赌局、token 赌注、表演性放水
- [~] 心理层：读对手风格——机制 + Agent 侧读人策略已完成（/match/read + read_opponent + llm-seat 读人策略，
  docs/match.md §7）；真模型首次读人实测方向命中 3/4、最佳误差 0.015°；安全球博弈/读人反制待做
- [ ] AI 解说员、师徒传承（校准笔记传递）
- [ ] 模式 C 渲染图像感知（多模态）
...
- 2026-08-30 M6.4 v1 完成：常驻大厅（多桌 + 动态认座 + handSeed 肌肉记忆 + 选桌观战；
  端到端冒烟：大厅自动分桌打完 12 杆并自动续局，每局独立公开日志可回放，docs/lobby.md）
- 2026-08-30 M6.5 Elo 榜完成：大厅终局自动入账（K=32 标准公式，零和），`--db` SQLite 持久、
  跨重启保留，大厅页渲染榜单（实测 bob 胜 alice → 1516/1484；docs/lobby.md §7）
- 2026-08-30 心理层 v1：读对手机制（隐藏 biasBase 计分 + 噪声限次防反推 + reads 入库）；
  活体验证：读人评分/方向判定/限次/入库全通（docs/match.md §7）
- 2026-08-30 读人实测：信号改用"意图角 vs 母球出射角"（cueHeading）；真模型首次读人方向命中 3/4、
  最佳误差 0.015°；修复驱动 trial/shot 字段错位（反馈/证据链此前未吃到数据）；llm-seat 读人策略 + 常驻续局
