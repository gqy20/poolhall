...
## 8. v9 两层记忆：自写校准笔记（2026-08-29，4-seed 验证）

**设计动机**：业界分层记忆（MemGPT/Letta core memory、Reflexion、Claude Code memory）
的共同轴是**结论/证据**而非旧/新。v8 系列所有形态都在喂证据（杆的罗列）；校准任务
真正需要的是结论（"我的手偏左约 1 球径，右修 0.5° 已见效"）——v6 碰巧通过多轮
推理原文做到了这一点。

**实现**：schema 加 `note` 字段（≤300 字），模型每杆改写一句话校准结论，harness
下一杆以 `<calibration_note>` 回放；证据层维持近窗 6。任务语义不变（发现仍由模型
自己做，harness 不代算统计量——保持 benchmark 效度）。

**4-seed 结果**（50 杆/seed）：

| | seeds 42/7/123/2024 | 均值 | 中断 | 跨 seed σ |
|--|---------------------|------|------|----------|
| v9（笔记） | 32/29/36/33 | 65% | **0/4** | **2.9** |
| v6（文本） | 41/35/27/36 | 70% | 0/4 | 5.9 |
| v8.2 | 34/30/9@17中断/35 | 66%* | 1/4 | — |

\* 完成 run。配对 v9−v6 = [−9,−6,+9,−3]，均值 −2.2 杆——噪声内无显著优劣。

**结论**：
1. 笔记不提升均值精度（三层 harness 65-70% 等价），但**一致性显著更好**：
   零中断 + 跨 seed 方差减半（σ 5.9→2.9）。
2. **新研究产物**：calibNote 是模型对自身偏差的自述，进研究日志——
   "自知之明准确率"（note 陈述方向 vs 注入 bias 真值）成为可计算的新指标，
   与知行曲线互补（知行曲线测行为收敛，note 测自我认知）。后置立项。
3. 分层记忆的调研判断被实验支持一半：结论层回放**无害且增益一致性**，
   但在此模型/任务上不转化为精度——精度瓶颈不在记忆组织（在端点 harness 与模型本身）。
