...
## 9. 自知之明准确率（2026-08-29 首测：知行分裂的直接量化）

**定义**：v9 calibNote 中的方向陈述（"偏左/right-drift"）与实测漂移真值的命中率。
真值 = 该局 miss 杆目标球终点相对袋口线的带符号横向偏移均值（与反馈 missNarrative
同轴同约定）。指标公式与实现待正式化（当前为分析脚本级）。

**4 局实测**（seed 42/7/123/2024，注入 bias 恒负 → 实测漂移恒为左）：

| seed | 方向陈述杆数 | 自知之明 | 复读基线 | 多数票基线 |
|------|------------|---------|---------|-----------|
| 42 | 24/50 | 46% | 38% | 42% |
| 7 | 28/50 | 75% | 57% | 64% |
| 123 | 20/50 | 80% | 50% | 45% |
...
**发现**：
1. **信息完备但认知不完备**：每杆反馈文本明示带符号方向（"横向偏左 N 球径"），
   模型综合出的方向自述仅 66% 正确——知道的比看到的少。
2. **超越平凡策略**（66% > 复读 52% > 多数票 49%）：模型在做真实的跨杆综合，
   不是复读最近反馈。
3. **知行分离的同框测量**：行为侧知行曲线收敛 7.81×（§5.4），认知侧方向仅 66%——
   "行"的补偿可以靠反馈-修正回路收敛，"知"的自我模型建设是独立且更难的瓶颈。

**口径注记**：方向提取为正则（17-28/50 杆有明确方向陈述）；真值取局级常数方向
（bias 恒负）；n=4 局。正式化需：结构化 note schema（方向枚举字段替代正则）、
逐时真值（对 OU 漂移）、多模型对照。
...
## 10. v9.x prompt 迭代（spin 触发率）

| 版本 | spin 段措辞 | 进球(s42) | err_i p50 | err_i p75 | spin.y 触发 |
|------|------------|------------|-----------|-----------|------------|
| v6   | 无 | 41/50 | — | — | 0 |
| v8.2 | 无 | 34/50 | 0.000° | 0.08° | 0 |
| v9.0 | 无 | 32/50 | 0.001° | 0.20° | 0 |
| **v9.1** | "直线球必设 spin.y<0（低杆）防 scratch" | **19/50** | 0.033° | **1.06°** | **50/50** |
| **v9.2** | "spin 不是必填……专注 aimX/aimY 精度" | **39/50** | **0.000°** | **0.007°** | 0/50 |

**结论**：prompt 替模型做"它不需要、也做不好"的事会害它。v9.1 强制低杆让模型注意力分散到 spin（p75 退化 13×），v9.2 删硬规则后精度和 v8.2/v6 持平。
spin 物理（v1/v2/v3）正确存在，但**校准任务场景不需要**——真正会用到 spin 的是清台走位（M6.1），
那里 scratch 威慑让 spin 变成生存技能。下一步：把 v9.2 spin 段推广到 clear.yaml（c2 同样有过度建议风险）。

### 10.1 v9.2 多 seed 验证（4 seed × 50 杆）

| 版本 | 4-seed 进球均值 | σ | 完成杆数 | err_i p75 中位 |
|------|----------------|---|---------|---------------|
| v8.2 | 24.7/50 | 13.8 | 3/4（123 中途崩） | 0.04° |
| v9 | 32.5/50 | 2.9 | 4/4 | 0.008° |
| **v9.2** | **36.5/50** | **3.7** | **4/4** | **0.005°** |

**v9.2 = 当前 harness 最强基线**：进球最高、方差最低、零中断、亚度精度。
**4-seed 复核推翻此前的 v6 假说**：v6=82% 是 seed-42 最佳；v6 四 seed 均值实际更低（被早期文件错位高估）。
真正结论：v8 harness（generateObject）相对 v6（text+parse）的真实差距 ≈ 8-9pp + 1/4 概率 run 中崩，**不是 14pp**。
