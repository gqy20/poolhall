...
### 7.5 缓存优化调查（v8.5：命中率与精度的互斥）

usage 统计（§7.6 落地）实测缓存命中率仅 6.1%（~128 tok/杆 = system+tools 前缀）。
机制调查与优化实验：

| 形态 | 命中率 | 进球率 | 机制 |
|------|--------|--------|------|
| v8.2 滑动窗口(6) 无断点 | 6.1% | **34/50** | 窗口滑动 → 消息首行每杆变，前缀在 system 后即断 |
| v8.5a 全量 append-only 无断点 | 9.8% | — | 前缀稳定但仍只命中自动缓存的 system 块 |
| v8.5b append-only + `cache_control:ephemeral` 断点 | **31.9%** | 27/50 | 命中随会话增长（25%→35%），128-token 块量化 |

结论：
1. 端点自动缓存仅覆盖 system+tools；**显式 ephemeral 断点 + append-only 前缀**是
   提升命中率的正确机制（6%→32%，省 ~26% 输入 token 成本）。
2. 但**全量历史让早期 miss 永驻视野，过补偿加剧**（68%→54%）——滑动窗口的遗忘
   反而是行为阻尼。单消息内嵌形态下缓存与精度互斥。
3. benchmark 精度优先 → 代码保留 v8.2 形态（窗口 6，无断点）。
4. 换官方端点后此互斥自然消解：messages 多轮累积本身就是 append-only 前缀
   （user/assistant 对永不改写），**精度（82%）与缓存（后段 50%+）同源获得**。
