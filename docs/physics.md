...
### v3 走位阈值化（2026-08-29 探索，无可观测效果）

- 物理：v1 sliding 段力矩公式 `ω̇_x = -α·u_y` 在 v≠0 时把 spin.x/y 卷到自然值
  （dev≈0）；rolling 段 lockRoll 阈值化（|ω-自然值|<5·spinStop 才锁）当前是接口预留
- 现实：spin.x=+1 起步，单球 simulate 终态 ω={0,0,0}，与 v0 一致
- 让 spin.x/y 走位需改 sliding 段衰减率（违反教科书公式→pooltool 对拍挂），留 v3.1 校准
- 库边 spin.x/y push 系数激进（spin.x=1 一次撞库即出界），未启用；同 v3.1 流程
- v0/v2 涌现测试 106/106 全绿，未引入回归
